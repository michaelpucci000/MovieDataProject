...
movie_links = [a['href'] for a in soup.select('a.ipc-title-link-wrapper[href^="/title/"]')]

# Print the count of URLs found
...
for link in movie_links:
    ...
    data['language'] = movie_soup.select_one('a.ipc-metadata-list-item__list-content-item.ipc-metadata-list-item__list-content-item--link[href^="/search/title?title_type=feature&primary_language="]').text.strip()
    ...
    rank_text = movie_soup.select_one('a.top-rated-link').text.strip()
    ...
    insert_query = """INSERT INTO IMDbIMDbTop250(title, year, director, cast, studio, country, language, genre, imdbRating, imdbWatched, imdbRank, source, masterMovieId)
                   VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
