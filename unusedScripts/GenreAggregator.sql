SELECT
    ...
    ROUND(SUM(imdbRanked) / NULLIF(COUNT(NULLIF(imdbRanked, NULL)), 0), 2) AS AvgIMDbRank,  
    ROUND(SUM(letterboxdRanked) / NULLIF(COUNT(NULLIF(letterboxdRanked, NULL)), 0), 2) AS AvgLetterboxdRank,  
...
FROM (
    ...
    FROM (
        SELECT
            COALESCE(lbimdb.genre, lblb.genre) AS genre,
